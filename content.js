...
function extractReadableText() {
  ...
  // Remove elements with specific classes that are likely ads or navigation
  const adLikeClasses = ['ad', 'advertisement', 'banner', 'sidebar', 'widget'];
  adLikeClasses.forEach((className) => {
  ...
  // If we found a good content area, extract from it
  if (bestContent) {
    contentText = bestContent.textContent.trim();
  } else {
    // Fallback to body text
    contentText = bodyClone.textContent.trim();
  }

  // Clean up the text
  contentText = contentText.replace(/\s+/g, ' ').trim();

  // Limit to reasonable length (10KB max)
  return contentText.substring(0, 10000);
}
...
function calculateReadability(text) {
  ...
  const avgCharactersPerWord = words.length > 0 ? characters / words.length : 0;
